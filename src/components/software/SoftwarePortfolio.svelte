<script lang="ts">
  ...
  // Autoplay Effect: Cycles ONLY the accordion items in the active tab every 5 seconds
  $effect(() => {
    activeTab;
    activeItemIndex;
...
<div class="w-full" role="region" aria-label="Software Portfolio">
    ...
    {#each capabilities as tab}
      ...
      >
        <span class="flex items-center justify-center gap-2">
          {#if tab.id === 'saas-products'}
            <Laptop size={18} />
          {:else if tab.id === 'enterprise-solutions'}
            <Smartphone size={18} />
          {:else}
            <ShieldCheck size={18} />
          {/if}
  ...
  <div
    class="bg-base-100/60 backdrop-blur-md rounded-3xl border border-base-content/10 shadow-xl overflow-hidden p-6 md:p-12 lg:p-16 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16"
  >
    <!-- Left Details: 7 cols -->
    <div class="lg:col-span-7 flex flex-col justify-start gap-6">
      <div>
        <!-- <span class="text-xs uppercase tracking-wider font-bold text-primary mb-2 block">
          {#if activeTab === 'saas-products'}
            Market-Ready Enterprise Control
          {:else if activeTab === 'enterprise-solutions'}
            Requirement-Driven On-Demand Engineering
          {:else}
