<script lang="ts">
  ...
  let caseStudyInterval: ReturnType<typeof setInterval>;
  ...
  function stopCaseStudyAutoplay() {
    if (caseStudyInterval) clearInterval(caseStudyInterval);
  }
  ...
  <div class="grid grid-cols-1 lg:grid-cols-5 gap-8 items-stretch">
    ...
    <div class="lg:col-span-3 flex flex-col h-full">
      {#if caseStudies.length > 0}
        ...
        <a 
          href={current.url || '#'}
          class="bg-success/5 border border-success/15 dark:border-success/10 rounded-4xl p-6 md:p-8 relative flex flex-col justify-between flex-1 min-h-[380px] shadow-xs group/card hover:border-success/30 hover:shadow-md transition-all duration-300 cursor-pointer select-none text-base-content hover:no-underline"
          onmouseenter={stopCaseStudyAutoplay}
          onmouseleave={startCaseStudyAutoplay}
        >
          ...
          <div>
            ...
            <div class="flex flex-col md:flex-row md:items-start justify-between gap-6">
              <div class="flex-1">
                {#key activeCaseStudyIndex}
                  <div in:fade={{ duration: 250 }}>
                    <h3 class="text-2xl font-bold tracking-tight text-base-content md:text-3xl max-w-lg mb-3 text-left">
                      {current.title}
                    </h3>
                    <p class="text-sm leading-relaxed text-base-content/75 md:text-base max-w-xl text-left">
                      {current.description}
                    </p>
                  </div>
                {/key}
              ...
              {#if current.image}
                <div class="w-full md:w-36 flex justify-center items-center shrink-0 max-h-32 mt-2">
                  <img 
                    src={current.image} 
                    alt={current.title} 
        ...
        <div class="flex items-center justify-center gap-4 mt-6">
          ...
          <div class="flex gap-1.5">
            {#each caseStudies as _, i}
              <button
                onclick={() => { activeCaseStudyIndex = i; startCaseStudyAutoplay(); }}
    ...
    <div class="lg:col-span-2 flex flex-col h-full">
      {#if testimonials.length > 0}
        ...
        <div 
          class="bg-base-100 border border-base-content/10 rounded-4xl p-6 md:p-8 relative flex flex-col justify-between flex-1 min-h-[380px] shadow-xs hover:shadow-md transition-all duration-300"
